...
def IsGomaRunning(options, goma_status_cmd):
  ...
  for line in output.splitlines():
    line = line.strip()
    if line.startswith(
        'compiler proxy ') and ' status: ' in line and line.endswith('ok'):
      return True
  return False
...
def BuildTargetFromCommand(options, command):
...
class Commands:
  ...
  def Clean_Command(self, options):
    ...
    stonesthrow.CheckCall(
        GetNinjaCommand(options) + ['-t', 'clean'] + options.targets, cwd=build_path)
  ...
  @Argument('--build', action='store_true', dest='build',
          help='build dependencies')
  @Argument('args', nargs=argparse.REMAINDER, metavar="ARGUMENTS",
          help="command to run")
  def Run_Command(self, options):
      ...
      if options.build:
          build_target = BuildTargetFromCommand(options, options.args[0])
...
def NeedsSource(options):
    ...
    return hasattr(options.method, 'needs_source') and options.method.needs_source
